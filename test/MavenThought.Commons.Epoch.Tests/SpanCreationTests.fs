...
module ``Span Creation tests`` =
    ...
    [<Property>]
    let ``When using minutes returns the minutes span`` (some:int) =
        some > 0 ==> (some .Minutes().Span = new TimeSpan(0, some, 0))

    [<Property>]
    let ``When using hours returns the hours span`` (some:int) =
        some > 0 ==> (some .Hours().Span = mkSpan some)

    [<Property>]
    let ``When using days returns the days span`` (some:int) =
        some > 0 ==> (some .Days().Span = mkSpan (some * 24))

    [<Property>]
    let ``When using weeks returns the weeks span`` (some:int) =
